...
exports.addCategory = async (req, res) => {
  try{
  const { categoryName, categorySection } = req.body;
  if(!categoryName) return res.status(401).json({error: 'Please enter category name'})
  if(!categorySection) return res.status(401).json({error: 'Please enter category section'})
    const category = await Category.findOne({categorySection, categoryName});
    if(category) return res.status(200).json({error: 'Category already exists'});
...
exports.getCategories = async (req, res) => {
  try{
    const { categorySection } = req.body;
    const categories = await Category.find({categorySection});
    return res.status(200).json({
      success: true,
      data: { categories }
    })
...
exports.getDocumentCategories = async (req, res) => {
  try{
    const categories = await Category.find({categorySection: 'document'});
    return res.status(200).json({
      success: true,
      data: { categories }
    })
...
exports.getCostCategories = async (req, res) => {
  try{
    const categories = await Category.find({categorySection: 'cost'});
...
exports.getInventoryCategories = async (req, res) => {
  try{
    const categories = await Category.find({categorySection: 'inventory'});
  ...
  try {
    ...
    if(type === 'fixtures'){
     vessel = await Vessel.findOne(
      ...
      vessel = await Vessel.findOne(
        {_id: req.body.vesselId},
        {documents:{$slice:[parseInt(limit) * (page - 1), parseInt(limit)]}},).exec();
        list = vessel.documents;
        docs = list.reduce(function (r, a) {
...
exports.getPaymentsOfManager = async(req, res) => {
  try {
    const { title, date, time, message, status } = req.body;
    ...
    if (!isExists) return res.status(404).json({ error: "Manager not found" });
     ...
     const payments = await Payment.find({managerId: req.data.id});
